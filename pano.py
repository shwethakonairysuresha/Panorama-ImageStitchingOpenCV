...
class Stitch:
    ...
    def __init__(self, args):
        root = Tk()
        labelText = StringVar()
        labelText.set("Enter images file name:")
        ...
        Button(root, text='Submit', command=self.start).pack(side=LEFT)
        entered_text = dirname.get()
    ...
    def showImage(self, string=None):
        if string == 'left':
            ...
            cv2.imshow("left image", cv2.resize(self.leftImage, (200, 200)))
        elif string == 'right':
            #cv2.imshow("right Image", self.rightImage)
            cv2.imshow("right image", cv2.resize(self.rightImage, (200, 200)))
        cv2.waitKey()
